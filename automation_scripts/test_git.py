# used gpt to generate 
# mimic's the local test in git 
import chess
import chess.engine
import math
import time
...
PRINT_MOVES = (
    os.getenv("PRINT_MOVES", "False") == "True"
)

PRINT_FINAL_BOARD = (
    os.getenv("PRINT_FINAL_BOARD", "False") == "True"
)
...
REPORT_DIR = "testing_reports"

os.makedirs(REPORT_DIR, exist_ok=True)

LOG_FILE = f"{REPORT_DIR}/{REPORT_NAME}.txt"
...
def material_difference(board):
    white = sum(
        len(board.pieces(pt, chess.WHITE)) * value
        for pt, value in PIECE_VALUES.items()
    )

    black = sum(
        len(board.pieces(pt, chess.BLACK)) * value
        for pt, value in PIECE_VALUES.items()
    )
...
with open(LOG_FILE, "a", encoding="utf-8") as f:
    ...
    f.write(f"PRINT_MOVES     : {PRINT_MOVES}\n")
    f.write(f"PRINT_FINAL_BOARD : {PRINT_FINAL_BOARD}\n")
    f.write("\n")
...
try:
    my = chess.engine.SimpleEngine.popen_uci(
        ["./chess", "uci"]
    )
