    ...
    while not board.is_game_over():
        ...
        if board.turn==chess.WHITE:
            if my_is_white:
                ...
                result=my.play(board, chess.engine.Limit(depth=GAME_DEPTH,time=TIME_LIMIT))
        ...
        if result.move is None or result.move not in board.legal_moves:
            ...
            move_limit_hit = True
            break
        print(f"Move {total_moves + 1:>3} | "f"FullMove={move_no:>3} | "f"Side={side_to_move:<5} | "f"Engine={engine_name:<9} | "f"Move={str(result.move):<5} | "f"Time={elapsed:.4f}s")
